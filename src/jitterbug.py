...
class Easy(object):
    ...
    def test_patterns(self, output=False):
        left_test = list(range(self.test_data.shape[0]))  # Convertendo para lista
        stats_test = {"tp": 0, "p": 0}
        for id in self.pattern_ids:
            left_test, stats = self.remove(self.test_data, self.y_label, left_test, id)
            stats_test["tp"] += stats["tp"]
            stats_test["p"] += stats["p"]

        print("Stats:", stats_test)  # Adicionar esta linha para visualizar as estatísticas

        # Verificar se o denominador é zero antes da divisão
        prec = 0.0
        if stats_test['p'] != 0:
            prec = float(stats_test['tp']) / stats_test['p']
        
        print("Precision:", prec)  # Adicionar esta linha para verificar a precisão

        # Salvar os dados "difíceis de encontrar"
        if output:
            rest_df = self.data[self.target].iloc[left_test]  # Usando left_test como índices
            with open("../new_data/rest/" + self.target + ".csv", "w", newline='\r\n') as file:
                rest_df.to_csv(file, index=False)

        return stats_test, prec
